...
class MLPDiffuser(nn.Module):
    # takes hidden_dim features, uses it to denoise out_channels features
    def __init__(self, hidden_dim, out_channels, input_size, patch_size, in_channels, hidden_size):
        super().__init__()
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        self.final_layer = FinalLayer(hidden_size, self.out_channels*(patch_size ** 2))

        self.mlp_diffuser = MLPDiffuser(hidden_size, self.out_channels*(patch_size ** 2), input_size, patch_size, in_channels, hidden_size)
    ...
    def initialize_weights(self):
        ...
        # nn.init.constant_(self.mlp_diffuser.mlp_in[0].weight, 0)
        # nn.init.constant_(self.mlp_diffuser.mlp_in[2].weight, 0)
        nn.init.constant_(self.mlp_diffuser.mlp_out[0].weight, 0)
        nn.init.constant_(self.mlp_diffuser.mlp_out[2].weight, 0)


    def denoise_step(self, x, c):
        # patchify
        # forward
        x = self.mlp_diffuser(x, c)
        # unpatchify
        x = self.unpatchify(x)
        return x
    ...
    def forward(self, noise,  y):
        ...
        x = self.x_embedder(noise) + self.pos_embed  # (N, T, D)
        y = self.y_embedder(y, self.training)    # (N, D)
        c = y                                # Combined condition embedding
        ...
        feats = self.final_layer(x, c)
        feats = self.unpatchify(feats)
        return feats
...
def test_model():
    ...
    # try to denoise
    feats = model.forward(x, y)
    print(feats.shape)

    # try to denoise step]
    t = torch.rand((2,))   
    x_0 = torch.randn(2, 4, 32, 32)
    x_1 = model.denoise_step(x_0, t)
    print(x_1.shape)
